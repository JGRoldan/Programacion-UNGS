...
func CrearStoreProcedureEntregaPedido(){
	//Conexion a BD
	...
	sqlFile, err := ioutil.ReadFile("data/store_procedure/sp_entrega_pedido.sql")
	if err != nil{
		log.Fatal(err)
	}
	...
	log.Println("-----------------------------------------------------")
	log.Println("Stored Procedure de Entrega Pedido creado correctamente")
	log.Println("-----------------------------------------------------")
}

func EntregaPedido() bool {
	...
	_, err = db.Exec("SELECT entrega_pedido($1)", idPedidoInputInt)
	if err != nil {
		log.Fatal(err)
	}
	...
	log.Println("--------------------------------------------------------")
	log.Println("Stored Procedure de Entrega Pedido ejecutado correctamente")
	log.Println("--------------------------------------------------------")
	return true
}